...
6. **Verify the chain from transcripts, not self-report.** Use the active host's thread or transcript API for each candidate. If the host exposes workspace-scoped transcript files, use only those paths. Never scan unrelated private histories. Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
