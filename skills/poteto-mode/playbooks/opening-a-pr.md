...
Opening a PR, pushing a branch, posting a comment, or changing a forge setting
requires explicit user authorization for that class of external action. Without
it, prepare the branch, commits, and PR text locally, then stop before the first
external mutation.
