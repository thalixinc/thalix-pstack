...
4. Verify each component against its baseline via image diff on the matching surface via the host's control tool. A nonzero diff is a fail. Investigate the pixel delta. Use the host's native repeat or goal mechanism per component until the diff is zero.
