...
3. Explore with host-native subagents under the Subagents section (the **guard-the-context-window** principle skill). Inherit the parent model by default. Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.
...
6. Run `pstack plan check <plan.md>` and fix every issue it reports (the **encode-lessons-in-structure** principle skill). The bundled `scripts/check-plan.mjs` remains available for upstream's stricter document-shape lint when Node.js is installed.
7. Hand back. Post the plan path and the command's output, then stop. Execution starts on the operator's explicit go, under the execution playbook the plan names.

**Verification.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked (the **prove-it-works** principle skill). That sentence is the verification rule. Every verification block opens with it. The live block is mandatory. Ten host-native lanes at the PR head drive the real surface through an installed control tool, per the **swarm** skill. Each lane is one box with a concrete scenario, the screenshot it saves, and its pass predicate. One lane is the **Regression lane against trunk.** It runs the same load-bearing scenario on trunk and head. If trunk does not have the feature, the lane records that fact and gates the behavior the diff adds plus the end state the user waits for instead of inventing a trunk result. The perf gate is dual-sided. Trunk and head must both produce the named metric. If trunk lacks the feature, also isolate the work the diff adds and set an absolute budget for that work plus the end-to-end state the user waits for. Do not claim a ratio between unlike scenarios. The perf block names the metric, the interleaved probe, the trunk baseline measured first, and the rule with the number that fails. A PR that changes an interaction is review-gated. The operator reviews it in chat with screenshots and a video before merge. A PR that changes no interaction writes `**Review gate.** None. <PR id> is not review-gated.` and no boxes under it.

**Control skill.** Pick an installed host-native tool for the actual surface: browser or UI control for web and desktop interfaces, terminal or PTY control for CLIs and TUIs, and the repository's simulator tool for native mobile. A PR that touches two surfaces gets lanes on both. A surface with no control tool is a risk in Appendix C, and its live block still names how each lane drives it.
...
- [ ] Arm the 30-minute audit tick with the host's native scheduler, wait, or goal mechanism. Never leave the cadence to memory.
...
- [ ] Run an installed cleanup or deslop skill before each commit and `no-comments` before review.
...
Each live lane runs in its own isolated environment at the PR head. Drive through the host's installed control tool for the surface.
...
**Verify, live.** Tests alone are not sufficient verification. A PR is verified only when its unit, live, and perf boxes are all checked. Ten host-native lanes run at the PR head, per the boot recipe.
...
- [ ] <With explicit user authorization to merge, the owner squash-merges its PR. Otherwise stop at merge-ready for the operator.>
