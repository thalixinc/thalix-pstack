...
This skill orchestrates three parts: an inline mining pass (see step 1), the active host's installed skill-authoring workflow, and the **unslop** skill (prose discipline). It sequences them. It does not replace them.

## Flow

### 0. Check for an existing skill

Look recursively in the active host's project and user skill roots for `*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory, not only at the top level. If one exists, confirm intent with the host's structured question tool when available, unless the user already said "update my skill" or similar.
...
Locate the current thread through the host's thread or transcript API before fanning out. If the host provides a workspace-scoped local transcript path, use only that path. Do not scan unrelated project histories.
...
Mining misses intent that has not come up yet. Use the host's structured question tool when available rather than asking the user to type from scratch.
...
Use the host's installed skill-authoring workflow to author the skill. Placement:

- Path: preserve an existing mode skill's category. For a new mode, use the host's documented project-local skill root unless the user explicitly prefers the host's user-level skill root.
