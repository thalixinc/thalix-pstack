...
1. Read `references/comment-sicko.md`, then spawn a host-native read-only reviewer with that prompt and the scope. If the host has registered the `Comment Sicko` agent, use it directly. Do not restate or weaken its rules.
