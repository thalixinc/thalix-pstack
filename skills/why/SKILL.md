...
Before spawning investigators, inspect the active host's available tools, apps, and MCP connectors. Do not scan private configuration directories or infer that an unavailable connector exists.
...
Each investigator uses a host-native research or exploration role with access to the required connector. The task itself remains read-only even when connector access requires an agent mode that is not labeled read-only. Inherit the parent model by default.
...
Spawn one synthesizer through the host's native agent interface. Give it connector access when it must spot-check citations, while keeping the task non-mutating. Do not assume a model identifier.
