...
Launch the reviewers concurrently through the host's native read-only review roles. Use at least two independent reviewers with distinct lenses. Inherit the parent model by default; use different model families only when the host confirms they are available. If delegation is unavailable, run the same lenses as separate sequential passes and disclose that limitation.
