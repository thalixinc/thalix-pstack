...
Use the active host's thread or transcript API and the workspace identity it provides. If the host exposes local transcript files, use only the current workspace path named by that host. Never scan another project's history to compensate for an unavailable transcript API.
