...
Prefer the native CLI. It stamps `ts`, writes the header on first use, strips stray tabs and newlines, and neutralizes spreadsheet-formula prefixes:
...
When the CLI is unavailable, use `scripts/log.sh <logfile> <phase> <decision> <why> <evidence> <result>` with the same behavior.
...
At the end of the run, before handing back, check the log told the truth. Read this run through the active host's thread or transcript API. If it provides a workspace-scoped transcript path, use only that path. Never scan unrelated private chats. Walk the log against what actually happened:
