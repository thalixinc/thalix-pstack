...
This upstream workflow requires Cursor and Grok Bot integrations. Run it only
when the active host exposes those exact integrations. It is retained for
source compatibility and is not part of the portable pstack core. Do not
simulate its external webhook, sender-key, or Tailscale actions when those
