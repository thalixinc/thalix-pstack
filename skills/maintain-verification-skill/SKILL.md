...
0. **Locate the target.** Find the verification skill to maintain: the `verify-*` skill under the active host's documented project-local skill root whose body has launch/drive sections and a feature map. Several candidates → ask which one; none → stop and point at `create-verification-skill` instead of inventing a target.
