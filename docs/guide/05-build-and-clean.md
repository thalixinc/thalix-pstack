...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs an installed cleanup or deslop workflow on the diff before each commit and applies [`unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. If the host has no cleanup skill, ask for the same outcome in plain words: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
