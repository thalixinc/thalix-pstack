...
use this host's scheduler or durable goal until done. if you're truly stuck after a few hours, stop and write up why.
```
...
- The host's scheduler, wait, or durable-goal mechanism re-checks the finish condition on events or a heartbeat. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) does not require Cursor's `/loop` command.
...
[Autopilot-full](../../skills/poteto-mode/playbooks/autopilot-full.md) runs a queue of independent PRs to merge-ready. It merges only when the current request explicitly authorizes that external action. Each PR gets one owner agent, and no owner merges on its own verdict. A swarm of fresh verifiers checks every merge-ready head; both explicit merge authorization and a clean verdict are required.
...
**Pitfall:** a duration is not a finish condition. "Work on this for 4 hours" gives the agent nothing to check, and you will wake up to four hours of motion instead of a result. Give the scheduler or durable goal a predicate that can pass or fail.
